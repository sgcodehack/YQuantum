...
The final quantum state $$\( |\Psi_3\rangle \)$$, which intricately encodes the input $$\( x \)$$ through amplitude and phase relationships established by the preceding circuit dynamics, is measured **once** in the computational basis.


#### Classical Bit String from Measurement

Let $$\( z \in \{0,1\}^n \)$$ represent the specific classical bit string obtained from this **single round of measurement** across all $$\( n \)$$ qubits. This measurement collapses the quantum state to a deterministic classical outcome for that specific computation. 

The exact value of $$\( z \)$$ is **highly sensitive to the original input $$\( x \)$$ ** due to the complex **entanglement** and input-dependent **rotations** in the quantum circuit. These dynamics are meticulously designed to induce **chaotic behavior** and ensure a strong **avalanche effect**.
...
This definition ensures that the hash output is **deterministic per execution instance**, a crucial property for reliable use in cryptographic protocols.


#### Functional Determinism for Verification

- **Determinism**: For any specific computational instance (e.g., hashing a specific block header), the measured bitstring $$\( z \)$$ represents a **unique and fixed result**.
- **Quantum Probabilism**: While repeated, independent runs of the process on the same input $$\( x \)$$ may yield different outcomes $$\( z' \)$$ due to quantum probabilistic behavior, $$\( H(x) \)$$ refers **only** to the result $$\( z \)$$ obtained from the original computation.
- **Verification Context**: In systems like blockchain, this functional determinism is critical. The validity check for $$\( H(x) \)$$ refers to the specific $$\( z \)$$ obtained during the original computation, ensuring a **fixed reference** for verification protocols.

#### Irreversibility and Cryptographic Quality

1. **Irreversibility from Measurement Collapse**:
   - The inherent **one-wayness** of the Superquantum Hash arises directly from the quantum measurement collapse, which irreversibly destroys the complex superposition and entanglement present in the original quantum state $$\( |\Psi_3\rangle \)$$.
   - This collapse, combined with the computational difficulty anticipated in inverting the complex unitary evolution:

     $$
     U = (\otimes R_z) \cdot U_{\text{ent}} \cdot (\otimes R_y)
     $$

     forms the basis for the hash function's intended **pre-image resistance**. Finding an input $$\( x \)$$ that yields a specific target $$\( z \)$$ should be computationally infeasible.

2. **Collision Resistance**:
   - The pseudo-randomizing dynamics generated by the **layered entanglement** and **rotations** are designed to map distinct inputs to nearly **orthogonal quantum states**.
   - This significantly minimizes the probability that two different inputs, $$\( x_1 \)$$ and $$\( x_2 \)$$, would collapse to the exact same output string $$\( z \)$$ in a single-shot measurement, thereby ensuring strong **collision resistance**.

3. **Output Uniformity (Goal)**:
   - Ideally, the outputs $$\( z \)$$ generated from diverse inputs should appear statistically **uniform** across the output space $$\( \{0,1\}^n \)$$, avoiding biases.
   - This uniformity is expected due to the thorough **state mixing** induced by the quantum circuit.

This measurement-based approach ensures that the Superquantum Hash delivers a unique combination of **determinism, irreversibility, pre-image resistance, collision resistance, and output uniformity**, making it suitable for secure cryptographic and blockchain applications.
---
